...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    // =========================================================
    // REPORTS EXPORT (Sub-issue #150)
    // Base: /api/reports/...
    // =========================================================
    if (segments[0] === "api" && segments[1] === "reports" && req.method === "GET") {
      const reportType = segments[2];
      const wantCsv = (query.format || "").toLowerCase() === "csv";

      // --- Leads Report ---
      if (reportType === "leads") {
        const docs = await Lead.find().sort({ clickedAt: -1 }).lean();
        const rows = docs.map((d) => ({
          platform: d.platform || "",
          clickedAt: d.clickedAt ? new Date(d.clickedAt).toISOString() : "",
        }));

        if (wantCsv) {
          return sendCsv(res, "leads_report.csv", toCsv(["platform", "clickedAt"], rows));
        }
        return sendJson(res, 200, rows);
      }

      // --- Campaigns Report (with aggregated metrics) ---
      if (reportType === "campaigns") {
        const campaigns = await Campaign.find().sort({ createdAt: -1 }).lean();

        // Aggregate totals per campaign
        const metricTotals = await CampaignMetric.aggregate([
          {
            $group: {
              _id: "$campaignId",
        ...
        const metricsMap = new Map(metricTotals.map((m) => [String(m._id), m]));

        const rows = campaigns.map((c) => {
          const m = metricsMap.get(String(c._id)) || {};
          return {
            name: c.name || "",
            platform: c.platform || "",
            budget: c.budget || 0,
        ...
        const headers = ["name", "platform", "budget", "spent", "totalRevenue", "totalClicks", "totalConversions", "startDate", "endDate", "status"];
        if (wantCsv) {
          return sendCsv(res, "campaigns_report.csv", toCsv(headers, rows));
        }
        return sendJson(res, 200, rows);
      }
      ...
      if (reportType === "referrals") {
        const docs = await Referral.find().sort({ createdAt: -1 }).lean();
        const rows = docs.map((d) => ({
          firstName: d.firstName || "",
          surName: d.surName || "",
          intake: d.intake || "",
          year: d.year || "",
          enrolledCourse: d.enrolledCourse || "",
          referredBy: d.referredBy || "",
          referralDate: d.referralDate ? new Date(d.referralDate).toISOString().slice(0, 10) : "",
          enrollmentStatus: d.enrollmentStatus || "",
          rewardStatus: d.rewardStatus || "",
        }));

        const headers = ["firstName", "surName", "intake", "year", "enrolledCourse", "referredBy", "referralDate", "enrollmentStatus", "rewardStatus"];
        if (wantCsv) {
          return sendCsv(res, "referrals_report.csv", toCsv(headers, rows));
        }
        return sendJson(res, 200, rows);
      }
      ...
      if (reportType === "visits") {
        const docs = await Visit.find().sort({ timestamp: -1 }).lean();
        const rows = docs.map((d) => ({
        ...
        if (wantCsv) {
          return sendCsv(res, "visits_report.csv", toCsv(["timestamp", "ipHash", "userAgent"], rows));
        }
        return sendJson(res, 200, rows);
      }

      return sendJson(res, 404, { message: "Unknown report type" });
    }
